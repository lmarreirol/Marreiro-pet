                  ...
                  <div style={{ background: svcColor + '10', borderRadius: '0 0 14px 14px', border: `1.5px solid ${svcColor}30`, borderTop: 'none', minHeight: 80, padding: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {svcRows.length === 0 ? (
                      ...
                      <div key={a.id} style={{ background: '#fff', borderRadius: 8, padding: '7px 10px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', borderLeft: `3px solid ${a.status === 'AWAITING_PAYMENT' ? '#e5e7eb' : STATUS_COLORS[a.status]}` }}>
                        ...
                        {isAdmin && a.unitId && <div style={{ fontSize: 10, color: '#bbb' }}>📍 {UNITS.find(u => u.id === a.unitId)?.name ?? a.unitId}</div>}
                        {a.notes && <div style={{ fontSize: 10, color: '#aaa', fontStyle: 'italic', marginTop: 2 }}>"{a.notes}"</div>}
                        <div style={{ display: 'flex', gap: 4, marginTop: 6, paddingTop: 5, borderTop: '1px solid #f1f5f9', alignItems: 'center' }}>
                          ...
                          <div style={{ display: 'flex', gap: 3, marginLeft: 'auto' }}>
                            {(['CONFIRMED', 'COMPLETED', 'CANCELLED'] as const).map(s => (
                              <button key={s} onClick={() => updateStatus(a.id, s)} title={STATUS_LABELS[s]}
                                style={{ width: 20, height: 20, borderRadius: 5, border: `1.5px solid ${a.status === s ? STATUS_COLORS[s] : '#e5e7eb'}`, background: a.status === s ? STATUS_COLORS[s] : '#f8fafc', cursor: a.status === s ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9 }}>

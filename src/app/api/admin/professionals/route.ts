...
export async function POST(req: NextRequest) {
  ...
  const { name, slug, unitId, services } = await req.json()
  ...
  const pro = await prisma.professional.create({ data: { name, slug: cleanSlug, unitId, active: true, services: services ?? [] } })
